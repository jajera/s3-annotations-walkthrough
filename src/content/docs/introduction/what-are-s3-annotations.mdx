...
<Aside type="caution">
  **AWS CLI** ≥ 2.35.6 — `list-object-annotations` and `get-object-annotation`
  are new in CLI v2. Older installs fail with an unknown operation. Run `aws
  --version` before the examples below.
</Aside>

<Aside type="caution">
  **boto3** ≥ 1.43.32 — ingest and API Lambdas use `put_object_annotation` /
  `get_object_annotation`. Older boto3 builds do not include the S3 Annotations
  API.
</Aside>
...
<Aside type="note">
  Reading or writing annotations requires `s3:GetObjectAnnotation` /
  `s3:PutObjectAnnotation` — object permissions alone are not enough.
</Aside>
...
<Aside type="note">
  This demo uses **API Gateway** to invoke Lambda; Lambda reads annotations via
  the S3 API. API Gateway does not expose S3 Annotations directly — your app
  code (here, the API Lambda) calls S3.
</Aside>
