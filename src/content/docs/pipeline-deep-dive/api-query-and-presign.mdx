...
<Aside type="caution">
  API Gateway has a **hard 30-second integration timeout** for Lambda. The API Lambda timeout is set to **29s** in `terraform/lambda.tf` — there is no headroom beyond that ceiling.
</Aside>
...
<Aside type="note">
  **Mitigations:** keep the default 7-day lookback for demos; enable the [optional DynamoDB mirror](/pipeline-deep-dive/optional-dynamodb-mirror/) for larger catalogs or frequent filtered queries; redeploy after pulling API handler changes (`cd terraform && terraform apply`).
</Aside>
